/**
 * Flow 2 — Order Create Shopify → WooCommerce (shop-cc)
...
export async function shopCustomerCreateRoute(app: FastifyInstance): Promise<void> {
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!verifyShopifyHmac(getRawBody(request), sig)) {
        request.log.warn({ sig, ip: request.ip }, '[shop-customer-create] HMAC inválido — assinatura rejeitada');
        void logError({ flow: 'shop-customer-create', error_message: 'HMAC inválido', payload: { sig: sig ?? '(vazio)', ip: request.ip } });
      ...
      if (deliveryId) {
        ...
        if (!isNew) {
          request.log.warn({ deliveryId }, '[shop-customer-create] delivery-id duplicado — descartado');
      ...
      try {
        const job = await ordersQueue.add('shop-customer-create', order);
        request.log.info({ jobId: job.id, email }, '[shop-customer-create] webhook enfileirado com sucesso');
        return reply.status(202).send({ queued: true, jobId: job.id, email });
      } catch (err) {
        request.log.error({ err }, 'Erro ao enfileirar shop-customer-create');
