...
async function createShopifyWebhook(topic: string, callbackUrl: string): Promise<string> {
  const mutation = `
...
async function createWooWebhook(topic: string, deliveryUrl: string): Promise<number> {
  const client = wooClient();
...
export async function dashboardWebhookRoutes(app: FastifyInstance): Promise<void> {
  /**
   * GET /dashboard/api/webhooks/status
   */
  app.get('/dashboard/api/webhooks/status', async (_request, reply) => {
    const domain = config.domain;
    if (!domain) return reply.status(400).send({ error: 'Variável DOMAIN não configurada no .env' });
  ...
  /**
   * POST /dashboard/api/webhooks/sync
   */
  app.post<{ Body: { platforms?: string[]; force?: boolean } }>(
    '/dashboard/api/webhooks/sync',
    async (request, reply) => {
      const domain = config.domain;
      if (!domain) return reply.status(400).send({ error: 'Variável DOMAIN não configurada no .env' });

      const platforms = request.body?.platforms ?? ['shopify', 'woocommerce'];
      ...
      return reply.status(hasError ? 207 : 200).send({ domain, results, force });
    },
  );
}
