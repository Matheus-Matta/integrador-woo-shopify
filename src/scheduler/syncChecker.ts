...
const INTERVAL_MS     = Number(process.env.SCHEDULER_INTERVAL_MS  ?? 30 * 60 * 1000); // 30 min
const LOOKBACK_HOURS  = Number(process.env.SCHEDULER_LOOKBACK_HOURS ?? 2);
const MAX_ORDERS      = 50;
...
async function runSyncCheck(): Promise<void> {
  ...
  console.log(`[Scheduler] ▶ Verificação de sincronização iniciada (lookback=${LOOKBACK_HOURS}h)`);
...
export function startSyncScheduler(): void {
  ...
  console.log(`[Scheduler] Iniciado — intervalo=${INTERVAL_MS / 60000} min, lookback=${LOOKBACK_HOURS}h`);
