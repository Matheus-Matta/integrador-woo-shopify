...
export function buildEpofwMeta(
  ...
  return {
    ...
    display_key: `${labelTitle} (&#82;&#36;${priceDisplay})`,
    display_value: 'Sim',
  };
}
...
export function buildCoupons(order: Record<string, unknown>): { code: string }[] {
  return arrayOf<{ code?: string }>(order?.discount_codes)
    .filter((c) => c && c.code)
