...
export async function getCouponByCode(
  ...
  code: string,
): Promise<WooCoupon | null> {
  const client = getClient(instance);
  const { data } = await client.get<WooCoupon[]>('/wp-json/wc/v3/coupons', {
    params: { code },
  });
  if (!Array.isArray(data) || data.length === 0) return null;
  const wanted = String(code).toLowerCase();
  const found = data.find((c) => String(c.code).toLowerCase() === wanted);
  return found ?? null;
}

/** Cria um cupom simples no WooCommerce. */
export async function createCoupon(
  instance: WooInstance,
  payload: Partial<WooCoupon> & { code: string; amount: string; discount_type: string },
): Promise<WooCoupon> {
  const client = getClient(instance);
  try {
    const { data } = await client.post<WooCoupon>('/wp-json/wc/v3/coupons', payload);
    return data;
  } catch (err: any) {
