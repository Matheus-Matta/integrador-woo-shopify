...
export default function ProductDetailPage({ params }: { params: Promise<{ sku: string }> }) {
  ...
  const { data, isLoading } = useEntityDetail('product', sku, { eventsPage, errorsPage, limitEvents: 20, limitErrors: 20 });
  ...
  return (
    <div className="flex flex-col gap-6 p-4 lg:p-6 animate-slide-up w-full">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">SKU: {data.entityId}</span>
          </div>
        </div>
      </div>
        ...
        <TabsContent value="events">
          <Card>
            <CardHeader className="pb-3 border-b border-border/50">
              ...
              <CardDescription>
                Todos os eventos processados para este produto.
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="errors">
          <Card className="border-destructive/20">
            <CardHeader className="pb-3 border-b border-destructive/20 bg-destructive/5">
              ...
              <CardDescription>
                Erros críticos ou de infraestrutura que afetaram a sincronização deste produto.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                  ...
                  {data.errors.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={3} className="text-center py-16 text-muted-foreground">
                        Nenhum erro registrado para este SKU.
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      {selectedRow && (
        <LogDetailModal row={selectedRow} type="product" onClose={() => setSelectedRow(null)} />
      )}
    </div>
  );
}
