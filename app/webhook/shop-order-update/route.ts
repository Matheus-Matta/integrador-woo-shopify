...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Log detalhado do HMAC
    if (!sig) {
      console.warn(`[Webhook] ⚠️ Assinatura HMAC ausente no cabeçalho x-shopify-hmac-sha256`);
    }

    if (!verifyShopifyHmac(buffer, sig)) {
      console.warn(`[Webhook] ❌ HMAC INVÁLIDO — Rejeitando requisição de ${ip}`);
      void logError({ 
    ...
    if (!shopifyOrderId) {
      console.warn(`[Webhook] ⚠️ Erro: shopify_id não encontrado no JSON recebido`);
      return NextResponse.json({ error: 'id do pedido é obrigatorio' }, { status: 400 });
    }

    // Registrar recebimento nos logs do MongoDB para rastreamento
    await logOrder({
    ...
    void logError({ 
        flow: 'shop-order-update', 
        error_message: `Erro crítico no /webhook: ${error.message}`, 
